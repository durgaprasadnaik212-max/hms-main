...
export default function Inventory() {
  ...
  const [formData, setFormData] = useState({ name: '', stock: '', expiryDate: '', supplierId: '' });
  ...
  const handleAdd = () => {
    setEditingId(null);
    setFormData({ name: '', stock: '', expiryDate: '', supplierId: '' });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ ...formData, stock: parseInt(formData.stock) })
    });
            ...
            <tbody className="divide-y divide-white/10">
              {Array.isArray(medicines) && medicines.map((med, index) => (
                <tr key={index} className="hover:bg-white/10 transition-colors">
                  <td className="px-6 py-4 font-medium text-white">{med.name}</td>
                  <td className="px-6 py-4 text-white/80">{med.stock}</td>
                  <td className="px-6 py-4 text-white/60">{new Date(med.expiryDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4 text-white/80">{med.supplier?.name || 'Unknown'}</td>
                  {canManageInventory && (
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => handleEdit(med)} className="p-1.5 text-white/40 hover:text-indigo-400 hover:bg-white/10 rounded transition-colors">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(med.id)} className="p-1.5 text-white/40 hover:text-red-400 hover:bg-white/10 rounded transition-colors">
              ...
              {medicines.length === 0 && (
                <tr>
                  <td colSpan={canManageInventory ? 5 : 4} className="px-6 py-8 text-center text-white/40">
                    No medicines found.
                  </td>
                </tr>
              )}
            ...
            >
              <div className="glass-card w-full max-w-md pointer-events-auto overflow-hidden">
                <div className="p-6 border-b border-white/20 bg-white/10 flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-white">{editingId ? 'Edit Medicine' : 'Add Medicine'}</h3>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-1.5">Medicine Name</label>
                  ...
                  <div>
                    ...
                    <input type="number" required value={formData.stock} onChange={(e) => setFormData({...formData, stock: e.target.value})} className="w-full px-4 py-2 glass-input rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 text-white placeholder:text-white/30" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-1.5">Expiry Date</label>
                    <input type="date" required value={formData.expiryDate} onChange={(e) => setFormData({...formData, expiryDate: e.target.value})} className="w-full px-4 py-2 glass-input rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-1.5">Supplier</label>
                    <select required value={formData.supplierId} onChange={(e) => setFormData({...formData, supplierId: e.target.value})} className="w-full px-4 py-2 glass-input rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 text-white">
                      <option value="" disabled className="bg-slate-900">Select a supplier</option>
                      {suppliers.map(s => (
