...
export default function Radiology() {
  ...
  const handleDelete = async (id: string) => {
    const res = await fetch(`/api/radiology-bookings/${id}`, { method: 'DELETE' });
    if (res.ok) {
      fetchBookings();
    } else {
      alert('Failed to delete radiology booking.');
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = editingBooking ? `/api/radiology-bookings/${editingBooking.id}` : '/api/radiology-bookings';
    ...
    if (res.ok) {
      ...
      alert('Failed to save radiology booking.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">Radiology Bookings</h2>
        </div>
        <button 
          onClick={handleAdd}
      ...
      {/* Table and Modal for Radiology Bookings */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-white/70">
            ...
            <tbody className="divide-y divide-white/10">
              ...
              {bookings.length === 0 && (
                <tr>
                  <td colSpan={user?.role === 'patient' || user?.role === 'doctor' ? 4 : 5} className="px-6 py-8 text-center text-white/40">
                    No bookings found.
                  </td>
                </tr>
              )}
            ...
            >
              <div className="glass-card w-full max-w-md pointer-events-auto overflow-hidden">
                <div className="p-6 border-b border-white/20 bg-white/10 flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-white">{editingBooking ? 'Edit Radiology Booking' : 'Book Radiology Slot'}</h3>
                  <button 
                    onClick={() => setIsModalOpen(false)}
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-white/70 mb-1.5">Radiology Test</label>
                    <input 
                      type="text" 
                      required
                      value={formData.test}
